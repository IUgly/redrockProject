...
public interface ScheduledDao {
     ...
     * @return
     */
    @Insert("insert into student_distance_rank set student_id=#{student_id},nickname=#{nickname},day_distance=#{distance},week_distance=#{distance},month_distance=#{distance},all_distance=#{distance},steps=#{steps},college=#{college},duration=#{duration},class_id=#{class_id} on DUPLICATE key update day_distance=#{day_distance},week_distance = week_distance + #{distance},month_distance=month_distance+#{distance},all_distance=all_distance+#{distance}")
    void updateDayDistanceScoreToStuMysql(RankInfo rankInfo);
    @Insert("insert into class_distance_rank set class_id=#{class_id},day_distance=#{distance},week_distance=#{distance},month_distance=#{distance},all_distance=#{distance},steps=#{steps},college=#{college},duration=#{duration} on DUPLICATE key update day_distance=#{day_distance},week_distance = week_distance + #{distance},month_distance=month_distance+#{distance},all_distance=all_distance+#{distance}")
